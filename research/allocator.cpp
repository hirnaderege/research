...
// Enhanced slab structure with reuse tracking
...
// Enhanced slab proxy with reuse support
...
struct defaultSlabProxy {
    ...
    // Enhanced claim that can reuse existing slabs
    __host__ __device__
    bool claim(SLAB_TYPE* slab, size_t objectSize){
        ...
        // Case 1: Fresh slab (state == 0)
        if(currentState == 0) {
            if(intr::atomic::CAS_system(&allocState, 0ull, startingState) != 0)
                return false;
        ...
        // Case 2: Empty slab with same object size (reuse case)
        ...
        if(currentSize == objectSize && currentCount == 0) {
            // Slab is already configured for this object size and is empty
            return true;
        }

        return false; // Slab is in use or has different object size
    }
    ...
    bool attemptMaskAlloc(allocMaskElem& elem, size_t& result){
        ...
        while(intr::bitwise::population_count(maskCopy) < SLAB_ELEM_BIT_SIZE){
            ...
            allocMaskElem newMask = intr::atomic::or_system(&elem, targetMask);

            if((newMask & targetMask) == 0){
                result = target;
                return true;
            }
            maskCopy = newMask;
...
// Enhanced slab arena with reuse support
...
class SlabArena {
    ...
    public:
        ...
        // Enhanced allocation with reuse preference
        __host__ __device__
        slabAddrType alloc(size_t objectSize = 0){
            slabAddrType bestFreeSlot = AdrInfo<slabAddrType>::null();
            
            // First pass: Look for empty slabs with matching object size (best reuse)
            if(objectSize > 0) {
                for(size_t i = 0; i < SLAB_COUNT; i++){
            ...
            // Second pass: Look for completely free slabs
            for(size_t i = 0; i < SLAB_COUNT; i++){
                if(proxies.arena[i].data.getSize() == 0) {
                    if(bestFreeSlot == AdrInfo<slabAddrType>::null()) {
                        bestFreeSlot = static_cast<slabAddrType>(i);
                    }
                }
            }
        ...
        // Allocation for specific object size with reuse priority
        __host__ __device__
        slabAddrType allocForSize(size_t objectSize){
        ...
        bool free(slabAddrType slabAddr){
            if(slabAddr >= SLAB_COUNT)
                return false;
            // Note: We don't immediately clear the slab state to enable reuse
            // The slab proxy handles this through its isEmpty() check
            return proxies.arena[slabAddr].data.clearAllocState();
        }

        // Get statistics about slab usage
        __host__ __device__
        void getStats(size_t& totalSlabs, size_t& usedSlabs, size_t& emptyReusableSlabs) {
            totalSlabs = SLAB_COUNT;
            usedSlabs = 0;
            emptyReusableSlabs = 0;
...
// Enhanced allocator with reuse support
...
class SimpleAllocator {
    ...
    public:
        ...
        __host__ __device__
        void* alloc(){
            // Use size-aware allocation for better reuse
        ...
        bool free(void* ptr){
            ...
            // Note: We don't free the slab immediately when emptied to enable reuse
            // The slab remains configured for this object size and can be reused directly
